<template>
  <el-container>
    ...
    </el-checkbox-group></el-row></el-main>
      ...
      <el-table :data="imgInfo">
        ...
        <el-table-column label="备份时间">
          <template slot-scope="scope">
            <el-tag size="medium"> {{ scope.row.created_on | parseTime('{y}-{m}-{d} {h}:{i}') }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="图床ID">
          <template slot-scope="scope">
            <el-tag size="medium" type="success">{{ scope.row.image.store_id }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column property="link" label="备份链接"/>
        <el-table-column label="原图链接">
          <template slot-scope="scope">
            <el-tag size="medium" type="success">{{ scope.row.image.link }}</el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog :visible.sync="imageLinkDialog" :fullscreen="fullScreen" title="链接合集">
      ...
      <el-table :data="checkedImages">
        <el-table-column label="图床 ID">
          <template slot-scope="scope">
            <el-tag size="medium" type="success">{{ scope.row.image.store_id }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column property="link" label="图片本地链接"/>
        <el-table-column label="操作">
          <template slot-scope="scope" >
            <el-button :data-clipboard-text="scope.row.image.link" class="tag-read" size="medium" plain @click="copy">复制原图链接</el-button>
            <el-button :data-clipboard-text="scope.row.link" class="tag-read" size="medium" plain type="primary" @click="copy" >复制本地链接</el-button>
      ...
      <el-row>
        <el-col>
          <el-card :body-style="{ padding: '0px' }">
            ...
            <div style="padding: 14px;">
              <el-button :data-clipboard-text="info.link" type="success" size="medium" class="tag-read" plain round @click="copy">复制本地链接</el-button>
              <el-button :data-clipboard-text="nowLink" type="primary" size="medium" class="tag-read" plain round @click="copy">复制原图链接</el-button>
...
import { getSyncImages, deleteSyncImageList } from '@/api/image'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getSyncImages(this.listQuery).then(response => {
    ...
    transformTo(tab, event) {
      ...
      switch (tab.label) {
        ...
        case 'MarkDown':
          this.checkedImages.forEach(e => {
            this.linksArea += '![' + e.image.name + '](' + e.link + ')' + '\n'
          })
          break
        case 'HTML':
          // this.linksArea = ''
          this.checkedImages.forEach(e => {
            this.linksArea += '<img  alt="' + e.image.name + '" src="' + e.link + '" />' + '\n'
          })
          break
        case 'BBCode':
          // this.linksArea = ''
    ...
    showSingleInfo(info) {
      this.info = info
      console.log(info.image.link)
      this.nowLink = info.image.link
